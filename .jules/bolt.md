...
## 2025-02-12 - Memoize list rows with React.memo in HostManager
**Learning:** Found a performance bottleneck where the `HostManager` component was rendering a long list of hosts by inlining `<tr>` elements within a `hosts.map()`. When an individual host entered edit mode, the parent state updated, triggering an O(N) re-render of the entire list.
**Action:** Extracted the list item rendering logic into a separate `HostRow` component wrapped with `React.memo()`. By selectively passing dynamically changing props (like `editForm`) only to the actively edited item and using `useCallback` for stable function references, `React.memo` effectively skips unnecessary renders for unmodified items, reducing DOM thrashing.
