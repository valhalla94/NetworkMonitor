...
## 2025-01-22 - Optimize React array state updates
**Learning:** Found a performance bottleneck where mapping over an array in a React state setter function creates a new array reference even if no items were actually modified (e.g. during frequent but identical SSE updates). This new array reference triggers a React re-render and forces any dependent `useMemo` hooks (such as those filtering or grouping the array) to recalculate unnecessarily.
**Action:** When updating array state in React (e.g., mapping over `prev` in a state setter), track if any elements were actually modified. If no changes occurred, return the original `prev` array reference instead of the new mapped array to prevent unnecessary re-evaluations of dependent `useMemo` hooks and component re-renders.
