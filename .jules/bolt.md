...
## 2026-09-18 - FileResponse with Temporary File Cleanup
**Learning:** Found a performance bottleneck where large datasets exported as CSVs were loading all records into memory using `.all()` and a massive `io.StringIO()` buffer, which could cause Out-Of-Memory (OOM) errors and block the main thread. When using `tempfile.NamedTemporaryFile` and returning a FastAPI `FileResponse`, an unhandled exception before the `FileResponse` returns can leave an orphaned temporary file.
**Action:** Always wrap temporary file writing inside a `try...except Exception` block. Close and delete the temporary file (`os.unlink`) in the `except` clause, and always ensure the file is closed in the `finally` block before returning the `FileResponse` with a `BackgroundTask` for cleanup.
