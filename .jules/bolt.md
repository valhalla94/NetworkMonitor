...
## 2025-02-18 - Optimize React Rendering in Maps\n**Learning:** Found a performance bottleneck where `Dashboard.jsx` rendered an array of hosts using `.map()`, and within that map, passed an inline arrow function (`onClick={() => setSelectedHost(host)}`) directly to a div. This causes the function to have a new reference on every render, preventing React from efficiently identifying stable nodes, and blocking proper memoization.\n**Action:** Extract list items rendered within a `.map()` loop into their own components and wrap them in `React.memo()`. To preserve shallow equality checks in the memoized component, avoid passing inline functions from the parent. Instead, pass stable references (like state setters) and primitive props, and handle the specific invocation logic inside the memoized child component.
