...
## 2025-02-12 - Prevent OOM and Thread Hazards on Large Exports
**Learning:** Found a major performance and stability issue in the `/export/metrics/{host_id}` endpoint. `db.query(...).all()` was loading the entire dataset into a single list of ORM tuples, and the response was streaming from an in-memory `io.StringIO()` buffer via `StreamingResponse`. For endpoints returning large time-series datasets, this leads to OOM (Out Of Memory) crashes and spikes SQLite thread contention.
**Action:** Always avoid `query.all()` for large dataset exports. Use `query.yield_per(1000)` to stream database results synchronously within the request thread, write them to a temporary file via `tempfile.NamedTemporaryFile`, and return a FastAPI `FileResponse` paired with a Starlette `BackgroundTask` to safely clean up the temporary file on disk after serving.
