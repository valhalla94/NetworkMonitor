...
## 2025-02-13 - Optimize Global Network Status Aggregation
**Learning:** Found a performance bottleneck in the `/status` endpoint where querying specific columns for all hosts and looping over them in Python to compute global reachability and latency caused an O(N) memory allocation and processing time (taking ~0.89s for 100k hosts).
**Action:** Always push aggregations down to the database using SQLAlchemy's `func.count()`, `func.sum()`, and `func.avg()` combined with `case()` statements for conditions. This reduces data transfer to exactly 1 aggregated row and leverages SQLite's optimized C-level computation (reducing time to ~0.02s for 100k hosts).
