...
## 2025-02-23 - Optimize React component with React.memo
**Learning:** Found a performance bottleneck where the `Dashboard.jsx` component was unnecessarily re-rendering individual host cards when completely unrelated state variables (like time range or SSE data for other hosts) were updated. Passing an inline arrow function (e.g. `onClick={() => setSelectedHost(host)}`) to a child list item component creates a new reference on every render, preventing `React.memo` from successfully blocking those re-renders.
**Action:** When mapping over items to render child list components, extract the child into a standalone component, wrap it with `React.memo`, and pass a stable reference to state setter functions (like `onSelect={setSelectedHost}`). Handle any specific iteration logic (like passing the specific list item argument) inside the child's internal handler function to maintain parent shallow equality.
