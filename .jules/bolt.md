...
## 2025-02-13 - Optimize CSV Export Memory Footprint
**Learning:** Found a major performance bottleneck where the `export_metrics_csv` endpoint fetched up to a year's worth of metrics data and loaded everything into memory using SQLAlchemy's `.all()` combined with an in-memory `StringIO` buffer. This could crash the server with O(N) memory allocation per request. Streaming it lazily via FastAPI's `StreamingResponse` creates a thread-safety error (`ProgrammingError`) with SQLite.
**Action:** When streaming large datasets from an SQLite database in FastAPI, always offload the data by synchronously iterating through chunks using SQLAlchemy's `query.yield_per()` and writing it to a temporary file via `tempfile` within the request thread. Serve the temporary file with a `FileResponse` and use a Starlette `BackgroundTask` to delete the file after the response finishes.
