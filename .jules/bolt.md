...
## 2025-02-13 - Prevent memory bottlenecks on large database row exports
**Learning:** Found a major performance bottleneck where the `/export/metrics/{host_id}` endpoint used `.all()` to fetch potentially hundreds of thousands of `PingResultDB` rows into memory at once, and then constructed a single massive CSV string in `io.StringIO()` before serving. This causes massive memory spikes and blocks thread execution during serialization on large timeframe exports.
**Action:** When exporting large datasets in FastAPI/SQLAlchemy, always use `.yield_per(chunk_size)` and stream the output using a Python generator wrapped in a `StreamingResponse`. Crucially, buffer the output rows in memory (e.g., using `io.StringIO` and chunking, yielding the buffer string) rather than yielding line-by-line, as yielding single lines from a synchronous generator causes massive thread pool thrashing in Starlette.
