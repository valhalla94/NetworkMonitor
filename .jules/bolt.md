...
## 2025-02-12 - Prevent FastApi/Starlette Generator Overhead in CSV Exports
**Learning:** Found a major performance bottleneck where the application fetched massive blocks of records (`.all()`) into a string buffer when generating CSVs. When I initially tried to convert it to a generator, yielding line-by-line from a synchronous generator (using `yield_per(1000)`) caused massive Starlette thread pool thrashing.
**Action:** When exporting large datasets to CSV in Starlette, use `yield_per(1000)` to reduce memory from the database, but make sure to buffer the lines into chunks of text (e.g. 1000 rows at a time) using `io.StringIO` before yielding them to the StreamingResponse. Also ensure that you explicitly yield the header row first so empty queries don't result in an empty file without headers.
