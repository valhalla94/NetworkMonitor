...
## 2024-02-23 - Add React.memo to prevent O(N) list item re-renders
**Learning:** Found a performance bottleneck where updating the `editForm` state in the `HostManager.jsx` component would re-render the entire table of hosts, even though only one host was actually being edited. This happens because the entire list mapping was inside the main component body, so every state change triggered a full re-render of all O(N) DOM elements for the list.
**Action:** Extract list item rendering logic into a separate `HostRow` component wrapped with `React.memo`. To ensure `React.memo` effectively skips renders for unmodified items, pass stable callback references using `useCallback` and selectively pass dynamically changing state props (like `editForm`) only to the item actively being edited.
