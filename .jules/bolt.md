...
## 2025-02-13 - Prevent Memory Exhaustion on Large CSV Exports
**Learning:** Found a critical performance vulnerability where exporting large datasets via CSV endpoints (e.g., fetching 90-365 days of metrics) called `.all()` to load all database rows into a massive string buffer in memory (`io.StringIO`) before streaming. This architecture guarantees an eventual Out of Memory (OOM) crash on production systems with extensive telemetry data.
**Action:** For large exports, implement batched database reads using `query.yield_per(1000)` and stream the output directly to a local disk using a temporary file (`tempfile.NamedTemporaryFile(delete=False)`). Serve the generated file via `FileResponse` and ensure cleanup via a `BackgroundTask(os.unlink)` to maintain a tiny memory footprint while protecting database connection pools.
