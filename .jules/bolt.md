...
## 2026-09-06 - Optimize CSV Export Streaming Payload
**Learning:** Found a performance bug where the streaming CSV export (`/export/metrics/{host_id}`) utilized a generator yielding line-by-line synchronously to `StreamingResponse`. Because Starlette routes synchronous generators to a thread pool for each `next()` call, yielding thousands of single lines causes massive thread pool thrashing and high CPU overhead, reversing the intended performance gain.
**Action:** Always buffer rows (e.g., using an internal chunk counter and `io.StringIO`) before yielding chunks to `StreamingResponse` when dealing with large datasets to minimize thread dispatch overhead.
