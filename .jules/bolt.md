...
## 2025-02-23 - Prevent Network Storms from Unstable Callbacks in React
**Learning:** Found a major performance bug in `Dashboard.jsx` where frequently changing UI state (like `selectedHost`) was included in the dependency array of `fetchHosts`, which was wrapped in `useCallback` and subsequently used in a `useEffect` dependency array. This caused a cascading network request storm: changing the selected host redefined `fetchHosts`, which triggered the `useEffect`, re-fetching all background dashboard data simultaneously on every selection. Furthermore, unmemoized callback handlers (like `handleRunSpeedTest` and `handleQuickPing`) passed to `React.memo` components (`SpeedTestCard`, `QuickPingCard`) bypassed memoization, causing full tree re-renders every 5s during SSE updates.
**Action:** Always use functional state updates (e.g., `setState(prev => ...)`) to eliminate frequently changing UI variables from `useCallback` dependency arrays, ensuring the callback reference remains stable. Additionally, ensure all event handlers passed to `React.memo` components are wrapped in `useCallback` to preserve shallow equality checks and skip unnecessary render cycles during high-frequency parent updates (like SSE loops).
