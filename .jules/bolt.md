...
## 2025-02-12 - Prevent Re-renders of React.memo Components from Unmemoized Props
**Learning:** Found a performance bottleneck where `Dashboard.jsx` passed unmemoized functions (`handleRunSpeedTest` and `handleQuickPing`) as props to child components (`SpeedTestCard` and `QuickPingCard`) wrapped in `React.memo()`. Because `Dashboard.jsx` updates its state frequently via a 5-second interval Server-Sent Events (SSE) connection, redefining these functions on every render broke the `React.memo()` shallow equality check, causing the children to needlessly re-render on every SSE tick.
**Action:** When a parent component receives high-frequency background updates (like SSE polling) and renders child components with `React.memo()`, ensure all callback functions passed as props are wrapped in `useCallback`. This keeps the callback reference stable and prevents unnecessary re-renders of the memoized child components.
