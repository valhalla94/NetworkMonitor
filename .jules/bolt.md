...
## 2026-08-02 - Memoize callback functions passed to React.memo child components
**Learning:** Found a performance bottleneck where components wrapped in `React.memo` (like `SpeedTestCard` and `QuickPingCard`) were unnecessarily re-rendering on every update to the parent `Dashboard` component (which updates every 5 seconds via SSE). The re-renders occurred because the parent passed unmemoized functions (`handleRunSpeedTest`, `handleQuickPing`, `fetchSpeedTestHistory`) as props. Unmemoized functions get new references on every render, breaking the shallow equality check of `React.memo`.
**Action:** Always wrap functions with `useCallback` when passing them down as props to child components that are optimized with `React.memo`. This ensures the function references remain stable across re-renders, preventing the child component from unnecessarily re-rendering.
