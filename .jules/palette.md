...
## 2026-09-22 - Focus Management for Disappearing Elements
**Learning:** When a button action causes the button itself to unmount (like a 'Clear search' or 'Clear filters' button), keyboard focus is lost and resets to the document body, causing a poor experience for screen reader and keyboard users.
**Action:** Use a `useRef` to explicitly return focus to a logical element (like the associated text input) when a button action results in its own unmounting.
