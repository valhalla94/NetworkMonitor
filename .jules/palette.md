...
## 2026-09-23 - Return focus to inputs after clearing
**Learning:** When building accessible React UI components where clicking a button unmounts the button itself (e.g., a 'Clear search' or 'Clear filters' button), keyboard focus is lost and resets to the document body, causing a poor experience for screen reader and keyboard users.
**Action:** Use a `useRef` to explicitly return keyboard focus to the associated input element to ensure a seamless experience.
