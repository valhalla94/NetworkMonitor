...
// ⚡ Bolt: Memoized HostRow prevents O(N) DOM re-renders of the host table when unrelated state (like typing in the Add Host form) changes.
const HostRow = React.memo(({ host, onEdit, onDelete }) => (
                            ...
                            {hosts.length === 0 ? (
                                ...
                                hosts.map((host) => (
                                    editingHost === host.id ? (
                                        ...
                                        <HostRow key={host.id} host={host} onEdit={startEdit} onDelete={handleDelete} />
                                    )
                                ))
                            )}
                        </tbody>
