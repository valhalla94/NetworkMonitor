...
const HostRow = React.memo(({ host, isEditing, editForm, setEditForm, saveEdit, cancelEdit, startEdit, handleDelete }) => {
    return (
                ...
                <>
                    ...
                    <td className="p-3 text-right align-top">
                        <div className="flex gap-2 justify-end">
                            <button onClick={() => saveEdit(host.id, editForm)}
...
const HostManager = ({ onHostAdded, hosts, onHostDeleted }) => {
    ...
    const saveEdit = useCallback(async (hostId, payloadForm) => {
        try {
            await updateHost(hostId, buildPayload(payloadForm));
            setEditingHost(null);
            onHostAdded?.();
        } catch (error) {
