...
// ⚡ Bolt: Extracted pure helper function outside component to keep reference stable
...
// ⚡ Bolt: Extracted list item into React.memo component to prevent O(N) re-renders
const HostRow = React.memo(({
    host,
    isEditing,
    editForm,
    setEditForm,
...
const HostManager = ({ onHostAdded, hosts, onHostDeleted }) => {
    ...
    const cancelEdit = useCallback(() => setEditingHost(null), []);

    const saveEdit = useCallback(async (hostId, currentEditForm) => {
        try {
            await updateHost(hostId, buildPayload(currentEditForm));
            setEditingHost(null);
            onHostAdded?.();
        } catch (error) {
                            ...
                            {hosts.length === 0 ? (
                                ...
                                hosts.map((host) => (
                                    <HostRow
                                        ...
                                        editForm={editingHost === host.id ? editForm : undefined}
                                        setEditForm={editingHost === host.id ? setEditForm : undefined}
                                        onSaveEdit={saveEdit}
