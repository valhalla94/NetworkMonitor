...
const HostFormFields = ({ f, setF, compact = false, disabled = false }) => {
    ...
    <div className={`space-y-4 ${compact ? 'text-sm' : ''}`}>
            ...
            <div className="col-span-2 space-y-1">
                ...
                <div className="relative">
                    ...
                    <input id={`${idPrefix}-name`} name="name" type="text" placeholder="e.g. Google DNS" value={f.name}
                        onChange={setField(setF)} required disabled={disabled}
        ...
        <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
                ...
                <select id={`${idPrefix}-monitor`} name="monitor_type" value={f.monitor_type} onChange={setField(setF)} disabled={disabled}
            ...
            <div className="space-y-1">
                ...
                <input id={`${idPrefix}-group`} name="group_name" type="text" placeholder="General" value={f.group_name}
                    onChange={setField(setF)} disabled={disabled}
        ...
        {(f.monitor_type === 'http') && (
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex items-end pb-2">
                    <label htmlFor={`${idPrefix}-ssl`} className={`flex items-center gap-2 ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
                        <input id={`${idPrefix}-ssl`} name="ssl_monitor" type="checkbox" checked={f.ssl_monitor} onChange={setField(setF)} disabled={disabled}
                            className="w-4 h-4 rounded border-slate-600 bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed" />
            ...
            {f.monitor_type !== 'heartbeat' && (
                <div className="space-y-1">
                    ...
                    <input id={`${idPrefix}-interval`} name="interval" type="number" placeholder="30" value={f.interval}
                        onChange={setField(setF)} min="5" disabled={disabled}
                        className="glass-input w-full px-3 py-2 rounded-lg outline-none text-sm" />
                </div>
            )}
            <div className="space-y-1">
        ...
        <div className="flex gap-4">
            <label htmlFor={`${idPrefix}-maint`} className={`flex items-center gap-2 ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
                <input id={`${idPrefix}-maint`} name="maintenance" type="checkbox" checked={f.maintenance} onChange={setField(setF)} disabled={disabled}
                    className="w-4 h-4 rounded border-slate-600 bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed" />
                <span className="text-xs text-slate-300">Maintenance Mode</span>
            </label>
            <label htmlFor={`${idPrefix}-enabled`} className={`flex items-center gap-2 ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
                <input id={`${idPrefix}-enabled`} name="enabled" type="checkbox" checked={f.enabled} onChange={setField(setF)} disabled={disabled}
                    className="w-4 h-4 rounded border-slate-600 bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed" />
...
// when the parent component (like HostManager or HostRow) re-renders,
// especially during state changes not directly affecting these specific fields.
// Expected Impact: Reduces re-renders of the large form component if props haven't changed.
