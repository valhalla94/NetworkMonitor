                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {groups[groupName].map(host => (
                            <button key={host.id} onClick={() => setSelectedHost(host)}
                                ...
                                className={`w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 glass-panel p-5 rounded-2xl cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl group relative overflow-hidden ${selectedHost?.id === host.id ? 'ring-2 ring-blue-500/50 bg-slate-800/60' : 'hover:bg-slate-800/60'}`}>
                                {host.maintenance && (
