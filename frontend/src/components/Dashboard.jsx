        ...
        const connectSSE = () => {
            ...
            es.addEventListener('hosts_update', (event) => {
                try {
                    ...
                    // Previously, incoming SSE updates would unconditionally spread into a new object:
                    // `{ ...host, ...updated }`.
                    // This creates a new reference for every host on every tick, invalidating React.memo
                    // on the child components. By shallow-comparing the incoming properties, we retain the
                    // existing object reference if nothing actually changed, saving significant render time.
                    setHosts(prev => {
                        ...
                        return prev.map(host => {
                            ...
                            for (const key in updated) {
                                if (host[key] !== updated[key]) {
                                    changed = true;
                                    break;
                                }
                            }
                            return changed ? { ...host, ...updated } : host;
                        });
                    });
