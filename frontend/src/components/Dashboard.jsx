...
// ⚡ Bolt: Memoized HostCard component.
// Using React.memo prevents this component from re-rendering unless its props change.
// By passing the stable `setSelectedHost` and extracting the inline function onClick handler
// `onClick={() => setSelectedHost(host)}` to a callback that calls `onSelect(host)` inside,
// we ensure the function reference doesn't change on every Dashboard render loop, satisfying
// React.memo shallow equality checks.
