...
def _get_sse_data():
    ...
    try:
        ...
        host_list = []
        for h in hosts:
            # ⚡ Bolt: Removed unused `latest_pings` database query block from this high-frequency SSE loop.
            # `last_ping` was fetched but never actually used in the payload below.
            host_list.append(
                {
                    "id": h.id,
                    "name": h.name,
